...
public class GetUnreadNotificationsQuery implements Query<Integer, List<NotificationGetDTO>> {
    private final NotificationRepository notificationRepository;

    @Override
    public ResponseEntity<List<NotificationGetDTO>> execute(Integer userId) {
        return ResponseEntity.ok(
                notificationRepository.findAllByUserIdAndReadFalseOrderByTimestampDesc(userId)
                        .stream()
